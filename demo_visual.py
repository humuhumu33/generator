"""
GA Mini Visual Demo - A simple demonstration of what Generator Architecture does

This demo shows the core concepts in an easy-to-understand way:
1. Input data (measurements)
2. Processing through sectors (constraints)
...
import numpy as np
import json
from ga_mini import ga_block, GAConfig, Sectors, PAGES, BYTES
...
def run_ga_processing(clean_data, noisy_data):
    """Run GA processing and show the results"""
    print(f"\n⚙️ Running Generator Architecture processing...")
    
    # Configure GA with different sector weights
    config = GAConfig(
        steps=50,  # More steps for better results
        step_size=0.1,
        sectors=Sectors(
            lambda_data=1.0,    # Strong emphasis on data fidelity
...
def analyze_results(clean_data, noisy_data, reconstructed, bhic):
    ...
    for sector, value in bhic.beta.items():
        status = "✅ EXCELLENT" if value < 0.01 else "⚠️ NEEDS WORK" if value < 0.1 else "❌ POOR"
    ...
    print(f"\n   🏆 Final Decision:")
    accepted = bhic.accept(eps=0.1)  # More lenient threshold for demo
    if accepted:
